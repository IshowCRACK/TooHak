...
import { RequestCreateQuizV2, clearUsers, registerUser, duplicateQuiz, infoQuiz, logoutUserHandlerV2, deleteQuestion, moveQuestion, updateQuiz ,createQuizQuestionHandlerV2} from './testHelpers';
...
describe('Tests related to creating a Quiz Question', () => {
  ...
  beforeEach(() => {
    ...
    defaultQuestionBody = {
      ...
      thumbnailUrl: "https://i.ebayimg.com/images/g/6f8AAOSwNe9dUpkA/s-l1200.webp"
    };
  });
  ...
  describe('Unsuccessful Tests', () => {
    ...
    test('QuizId does not refer to a valid quiz that this user owns', () => {
      ...
      expect(() => createQuizQuestionHandlerV2(quizId, tokenToJwt(userToken2), defaultQuestionBody)).toThrow(HTTPError[400]);
			});
    

    test('Question string is wrong format ', () => {
      defaultQuestionBody.question = 'abcd';
      expect(() => createQuizQuestionHandlerV2(quizId, userJwt, defaultQuestionBody)).toThrow(HTTPError[400]);
    ...
    test('There is no correct answers', () => {
      defaultQuestionBody.answers[0].correct = false;
		...
		test('Token is not a valid structure', () => {
			expect(() => createQuizQuestionHandlerV2(quizId, {token: "1"}, defaultQuestionBody)).toThrow(HTTPError[401]);
    });

		test('Provided token is valid structure, but is not for a currently logged in session', () => {
			logoutUserHandlerV2(userJwt);
			expect(() => createQuizQuestionHandlerV2(quizId, userJwt, defaultQuestionBody)).toThrow(HTTPError[402]);
    });
  });
