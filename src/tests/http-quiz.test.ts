...
import { registerUserHandler, logoutUserHandler } from './http-auth.test';
import { tokenToJwt } from '../token';
import { createQuizHandler, removeQuizHandler, clearUsersHandler, listQuizHandler, updateNameQuizHandler, infoQuizHandler, quizTransferHandler, updateDescriptionQuizHandler, viewQuizTrashHandler, trashRestoreQuizHandler, emptyTrashHandler } from './testHelpers';
...
describe('Quiz Update Description', () => {
  ...
  beforeEach(() => {
    token0 = registerUserHandler('JohnSmith@gmail.com', 'Password123', 'Johnny', 'Jones') as Token;
    token1 = registerUserHandler('JoeMama@gmail.com', 'Password456', 'Joe', 'Mama') as Token;
    quizId0 = (createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description 0') as AdminQuizCreate).quizId;
    quizId1 = (createQuizHandler(tokenToJwt(token1), 'Quiz1', 'Description 1') as AdminQuizCreate).quizId;
  });

  describe('Successful test', () => {
    beforeEach(() => {
      quizEditedTime = Math.round(Date.now() / 1000);
      res = updateDescriptionQuizHandler(tokenToJwt(token0), 'Updated description', quizId0) as OkObj;
      quizInfo = infoQuizHandler(tokenToJwt(token0), quizId0) as AdminQuizInfo;
    });
  ...
  describe('Unsuccessful test', () => {
    test('1. Returns an error when Quiz ID does not refer to a valid quiz', () => {
      res = updateDescriptionQuizHandler(tokenToJwt(token0), 'Updated description', -9);
    ...
    test('2. Quiz ID does not refer to a quiz that this user owns', () => {
      res = updateDescriptionQuizHandler(tokenToJwt(token0), 'Updated description', quizId1);
    ...
    test('3.Description is more than 100 characters in length', () => {
      res = updateDescriptionQuizHandler(tokenToJwt(token0), 'This is a description that is more than 100 characters long. It should trigger an error. ?!@ #$& *%)_@ ;-))1', quizId0);
      expect(res).toStrictEqual({ error: 'Description must be under 100 characters' });
    });

    test('6. Token is not a valid structure', () => {
      res = res = updateDescriptionQuizHandler({ token: '-1' }, 'Updated description', quizId0);
    ...
    test('7. Not token of an active session', () => {
      ...
      res = updateDescriptionQuizHandler(tokenToJwt(token1), 'Updated description', quizId1);
...
describe('Quiz Update Name', () => {
  ...
  describe('Successful test', () => {
    beforeEach(() => {
      quizEditedTime = Math.round(Date.now() / 1000);
      res = updateNameQuizHandler(tokenToJwt(token0), 'Quiz 0 Updated', quizId0) as OkObj;
      quizInfo = infoQuizHandler(tokenToJwt(token0), quizId0) as AdminQuizInfo;
    });
  ...
  describe('Unsuccessful test', () => {
    test('1. Returns an error when Quiz ID does not refer to a valid quiz', () => {
      res = updateNameQuizHandler(tokenToJwt(token0), 'Quiz 0 Updated', -9);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a valid quiz' });
    });

    test('2. Quiz ID does not refer to a quiz that this user owns', () => {
      res = updateNameQuizHandler(tokenToJwt(token0), 'Quiz 1 Updated', quizId1);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a quiz that this user owns' });
    });

    test('3. Non alphanumeric name entered', () => {
      res = updateNameQuizHandler(tokenToJwt(token0), '~ Quiz 0 Updated ~', quizId0);
      expect(res).toStrictEqual({ error: 'Must use only alphanumeric characters or spaces in name' });
    });

    test('4. Less than 3 characters', () => {
      res = updateNameQuizHandler(tokenToJwt(token0), 'hi', quizId0);
      expect(res).toStrictEqual({ error: 'Name must be between 3 and 30 characters' });
    });

    test('5. More than 30 characters', () => {
      res = updateNameQuizHandler(tokenToJwt(token0), 'abcdefghijklmnopqrstuvwxyz12345', quizId0);
      expect(res).toStrictEqual(
        { error: 'Name must be between 3 and 30 characters' });
    });

    test('6. Name already used by same user', () => {
      updateNameQuizHandler(tokenToJwt(token0), 'Quiz 0 Updated', quizId0);
      res = updateNameQuizHandler(tokenToJwt(token0), 'Quiz 0 Updated', quizId0);
      expect(res).toStrictEqual({ error: 'Quiz name is already in use' });
    });

    test('7. Token is not a valid structure', () => {
      res = updateNameQuizHandler({ token: '-1' }, 'Quiz 0 Updated', quizId0);
      expect(res).toStrictEqual({ error: 'Token is not a valid structure' });
    });

    test('8. Not token of an active session', () => {
      logoutUserHandler(tokenToJwt(token1));
      res = updateNameQuizHandler(tokenToJwt(token1), 'Quiz 1 Updated', quizId1);
      expect(res).toStrictEqual({ error: 'Token not for currently logged in session' });
    });
  });
});
...
describe('Quiz Info', () => {
  ...
  beforeEach(() => {
    ...
    token1 = registerUserHandler('JoeMama@gmail.com', 'Password456', 'Joe', 'Mama') as Token;
    quizId0 = (createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description 0') as AdminQuizCreate).quizId;
    quizId1 = (createQuizHandler(tokenToJwt(token0), 'Quiz1', 'Description 1') as AdminQuizCreate).quizId;
    quizId2 = (createQuizHandler(tokenToJwt(token1), 'Quiz2', 'Description 2') as AdminQuizCreate).quizId;
    logoutUserHandler(tokenToJwt(token1));
  });
  ...
  describe('Successful tests', () => {
    ...
    test('1. valid token and quizId for 1 owned quiz', () => {
      res = infoQuizHandler(tokenToJwt(token0), quizId0) as AdminQuizInfo;
      expect(res).toEqual(
    ...
    test('2. valid token and quizId for multiple owned quiz', () => {
      res = infoQuizHandler(tokenToJwt(token0), quizId0) as AdminQuizInfo;
      res0 = infoQuizHandler(tokenToJwt(token0), quizId1) as AdminQuizInfo;
  ...
  describe('Unsuccessful tests', () => {
    test('1 Invalid QuizId ', () => {
      res = infoQuizHandler(tokenToJwt(token0), -99);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a valid quiz' });
    });

    test('2. Quiz ID does not refer to a quiz that this user owns', () => {
      res = infoQuizHandler(tokenToJwt(token0), quizId2);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a quiz that this user owns' });
    });
    test('3. Not token of an active session', () => {
      res = infoQuizHandler(tokenToJwt(token1), quizId1);
      expect(res).toStrictEqual({ error: 'Token not for currently logged in session' });
    });

    test('4. Token is not a valid structure', () => {
      res = infoQuizHandler({ token: '-1' }, quizId1);
      expect(res).toStrictEqual({ error: 'Token is not a valid structure' });
    });
  });
});
...
describe('Quiz Remove', () => {
  ...
  describe('Successful Tests', () => {
    test('1. Successful Quiz Remove for User', () => {
      res = removeQuizHandler(tokenToJwt(token0), quizId0);
      expect(res).toStrictEqual({
    ...
    test('2. Successfull Quiz Create multiple for User', () => {
      res = removeQuizHandler(tokenToJwt(token0), quizId0);
      res0 = removeQuizHandler(tokenToJwt(token0), quizId1);
      expect(res).toStrictEqual({
  ...
  describe('Unsuccessful Tests', () => {
    test('3. Not token of an active session', () => {
      res = removeQuizHandler(tokenToJwt(token1), quizId1);
      expect(res).toStrictEqual({ error: 'Token not for currently logged in session' });
    });

    test('4. Invalid QuizId ', () => {
      res = removeQuizHandler(tokenToJwt(token0), -99);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a valid quiz' });
    });

    test('5. Quiz ID does not refer to a quiz that this user owns', () => {
      res = removeQuizHandler(tokenToJwt(token0), quizId2);
      expect(res).toStrictEqual({ error: 'Quiz ID does not refer to a quiz that this user owns' });
    });
  });
});
...
describe('Quiz Create', () => {
  ...
  describe('Successful Tests', () => {
    test('1. Successfull Quiz Create for User', () => {
      res = createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description0');
      expect(res).toStrictEqual({ quizId: 0 });
    });

    test('2. Successfull Quiz Create multiple for User', () => {
      res = createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description0');
      res0 = createQuizHandler(tokenToJwt(token0), 'Quiz1', 'Description0');
      expect(res).toStrictEqual({ quizId: 0 });
  ...
  describe('Unsuccessful Tests', () => {
    test('3. Not token of an active session', () => {
      res = createQuizHandler(tokenToJwt(token1), 'Quiz1', 'Description1');
      expect(res).toStrictEqual({ error: 'Token not for currently logged in session' });
    });

    test('4. Invalid Name ', () => {
      res = createQuizHandler(tokenToJwt(token0), '', 'Description0');
      expect(res).toStrictEqual({ error: 'A name must be entered' });
    });

    test('5. Invalid Name ', () => {
      res = createQuizHandler(tokenToJwt(token0), '12', 'Description0');
      expect(res).toStrictEqual({ error: 'Name must be between 3 and 30 characters' });
    });

    test('6. Invalid Name ', () => {
      res = createQuizHandler(tokenToJwt(token0), 'abcde12345abcde12345abcde12345sdf', 'Description0');
      expect(res).toStrictEqual({ error: 'Name must be between 3 and 30 characters' });
    });
    ...
    test('8. Invalid Description ', () => {
      res = createQuizHandler(tokenToJwt(token0), 'Quiz0', '12345678910123456789101234567891012345678910123456789101234567891012345678910123456789101234567891012345678910343142432dfasdf');
      expect(res).toStrictEqual({ error: 'Description must be under 100 characters' });
    });

    test('10. Name already used', () => {
      createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description0');
      res = createQuizHandler(tokenToJwt(token0), 'Quiz0', 'Description0');
      expect(res).toStrictEqual({ error: 'Quiz name is already in use' });
    });
  });
...
describe('adminQuizList tests', () => {
  ...
  describe('Unsuccessful test', () => {
    test('Invalid user', () => {
      ...
      expect(listQuizHandler(tokenToJwt(token))).toStrictEqual({ error: 'Token not for currently logged in session' });
    });
  });

  describe('Successful tests', () => {
    test('User with no owned quizzes', () => {
      expect(listQuizHandler(jwt)).toStrictEqual({ quizzes: [] });
    });

    test('User with one owned quizzes', () => {
      createQuizHandler(jwt, 'Countries of the world', 'Quiz on all countries');
      expect(listQuizHandler(jwt)).toStrictEqual({ quizzes: [{ quizId: 0, name: 'Countries of the world' }] });
    });

    test('User with multiple owned quizzes', () => {
      createQuizHandler(jwt, 'Countries of the world', 'Quiz on all countries');
      createQuizHandler(jwt, 'Flags of the world', 'Quiz on all flags');
      expect(listQuizHandler(jwt)).toStrictEqual({ quizzes: [{ quizId: 0, name: 'Countries of the world' }, { quizId: 1, name: 'Flags of the world' }] });
    });
  });
});
...
describe('Quiz Transfer', () => {
  ...
  beforeEach(() => {
    ...
    userToken2 = registerUserHandler('JaneAusten@gmail.com', 'Password123', 'Jane', 'Austen') as Token;

    userJwt2 = tokenToJwt(userToken2);
    quizId = (createQuizHandler(userJwt, 'Countries of the world', 'Quiz on all countries') as AdminQuizCreate).quizId;
  });
  ...
  describe('Unsuccessful Tests', () => {
    ...
    test('Quiz name is already used by user', () => {
      createQuizHandler(userJwt2, 'Countries of the world', 'Quiz on all countries V2');
  ...
  describe('Successful Tests', () => {
    ...
    test('Multiple Quiz Transfers with Multiple Quizzes', () => {
      const quizId2 = (createQuizHandler(userJwt2, 'Flags of the world', 'Flags on all countries') as AdminQuizCreate).quizId;
      expect(quizTransferHandler(userJwt, 'JaneAusten@gmail.com', quizId)).toEqual({});
...
describe('View Quiz Trash', () => {
  ...
  describe('Successful tests', () => {
    ...
    test('Non empty trash', () => {
      ...
      removeQuizHandler(userJwt, quizId);
      expect(viewQuizTrashHandler(userJwt)).toEqual({
        quizzes: [
          {
            quizId: quizId,
      ...
      removeQuizHandler(userJwt2, quizId2);
...
describe('Trash Restore Quiz', () => {
  ...
  beforeEach(() => {
    ...
    userJwt2 = tokenToJwt(userToken2);
    quizId = (createQuizHandler(userJwt, 'Countries of the world', 'Quiz on all countries') as AdminQuizCreate).quizId;
    quizId2 = (createQuizHandler(userJwt2, 'Flags of the world', 'Flags on all countries') as AdminQuizCreate).quizId;
    removeQuizHandler(userJwt2, quizId2);
  });
...
describe('Empty Trash', () => {
  ...
  beforeEach(() => {
    ...
    userJwt2 = tokenToJwt(userToken2);
    quizId = (createQuizHandler(userJwt, 'Countries of the world', 'Quiz on all countries') as AdminQuizCreate).quizId;
    quizId2 = (createQuizHandler(userJwt, 'Flags of the world', 'Flags on all countries') as AdminQuizCreate).quizId;
    quizId3 = (createQuizHandler(userJwt2, 'Continents of the world', 'Flags on all continents') as AdminQuizCreate).quizId;
    removeQuizHandler(userJwt, quizId);
    removeQuizHandler(userJwt, quizId2);
    removeQuizHandler(userJwt2, quizId3);
  });
